...
# Rename the columns
colnames(df_selected) <- c("Number of Firms", "Version", "Q-Nash", "Q-Mean", "r", 
...
kable(df_selected, format = "html", escape = FALSE) %>%
  ...
  column_spec(3, background = "#FFE0B2") %>%  # Color the third column
  column_spec(4, background = "#E1BEE7") %>%  # Color the fourth column
  ...
  column_spec(8, background = "#B3E5FC") %>%  # Color the eighth column
  column_spec(9, background = "#C8E6C9")      # Color the ninth column
...
df_selected[3, 5] <- cell_spec(df_selected[3, 5], bold = TRUE)
df_selected[4, 4] <- cell_spec(df_selected[4, 4], bold = TRUE)
df_selected[4, 2] <- cell_spec(df_selected[4, 2], bold = TRUE)
df_selected[6, 8] <- cell_spec(df_selected[6, 8], bold = TRUE)
...
# Rename the columns
colnames(df_selected) <- c("Variation", "Q-Optimum", "Q-Mean", "r", 
                           "Q-Mean 17-25", "r 17-25", "Pi-Maximum", "Ø Pi-Experiment") 
